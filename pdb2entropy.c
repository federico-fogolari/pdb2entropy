...
int main(int argc, char *argv[]) {
      ...
      int i, j,ii,jj,k, kk, l, m, n_tors,n_atoms_per_model,n_res_per_model,n_res_per_model_mi,n_tors_per_model,K, num_threads,*group2res;
      double **phit,c,L;
...
printf("%i residues read in %s\n", defs.n_res, flag_par.file_in_def);
         ...
         if(system.n_residues % system.n_models == 0 && system.n_atoms % system.n_models == 0)
               ...
               ent_x_report.ent_res=calloc(n_res_per_model, sizeof(double));
               ent_x_report.sd_ent_res=calloc(n_res_per_model, sizeof(double));
               ent_x_report.dmean_res=calloc(n_res_per_model, sizeof(double));
               ent_x_report.ndof_res=calloc(n_res_per_model, sizeof(int));
...
ent_k_2[k-1] = ent_k_2[k-1] * (double) tors_res[m].n_ang * (double) tors_res[m].n_ang / (double) system.n_models;
}
...
if(k == (ent_x_report.kn - 1))
 {
 ent_x_report.ent_res[m] = ent_k[k]; 
 ent_x_report.sd_ent_res[m] = sd_k[k]; 
 ent_x_report.dmean_res[m] = d_mean[k+1];
 }
}
}
...
entropy.mstw = calloc(n_res_per_model_mi, sizeof(double));
...
ent_k_2[k-1] = ent_k_2[k-1] * (double) tors_mi[m].n_ang * (double) tors_mi[m].n_ang / (double) system.n_models;
}
...
if(k == ent_x_report.kn - 1)
{
ent_x_report.ent_res[group2res[m]] = ent_x_report.ent_res[group2res[m]] + ent_k[k-1];
ent_x_report.sd_ent_res[group2res[m]] = ent_x_report.sd_ent_res[group2res[m]] + sd_k[k-1] * sd_k[k-1];
ent_x_report.dmean_res[group2res[m]] = ent_x_report.dmean_res[group2res[m]] + d_mean[k] * d_mean[k];
...
fprintf(fp_out_1,"ent_k %5i %9.2lf +/- %9.2lf   %9.2lf --- %5i %4s %5i %c %c %4s\n", k+1, ent_k[k], sd_k[k], d_mean[k+1], m,
tors_mi[m].res_name,
tors_mi[m].res_n,
tors_mi[m].res_ins,
tors_mi[m].chain,
